...
// geny has deprecated Scala 2.11 since 1.1.0
val scalaCrossVersions = List("2.12.17", "2.13.11", "3.3.3") ++ dottyVersion

val scalaNative = "0.5.5"

object requests extends Module {

  object jvm extends Cross[RequestsJvmModule](scalaCrossVersions)
  trait RequestsJvmModule extends RequestsModule {
    object test extends ScalaTests with RequestsTestModule
  }

  object native extends Cross[RequestsNativeModule](scalaCrossVersions)
  ...
  def mimaPreviousVersions =
    (
      Seq("0.7.0", "0.7.1", "0.8.2") ++
        Option
          .when(VcsVersion.vcsState().commitsSinceLastTag != 0)(
            VcsVersion.vcsState().lastTag
          )
          .flatten
    ).distinct
  override def mimaBinaryIssueFilters = Seq(
    ProblemFilter.exclude[ReversedMissingMethodProblem](
      "requests.BaseSession.send"
    ),
