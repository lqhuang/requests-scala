    ...
    for (chunkedUpload <- Seq(true, false)) {
      ...
      test("cookies") {
        ...
        test("space") {
          ...
          assert(
            read(res2) == Obj(
              "cookies" -> Obj("freeform" -> "test test", "hello" -> "hello, world")
            )
          )
        }
      }
      ...
      test("decompress") {
        ...
        assert(
          read(new String(res3.bytes))("data").toString ==
      ...
      /** Compress with each compression mode and call server. Server expands and passes it back so
        * we can compare
        */
      ...
      test("duplicateHeaders") {
        val res =
          requests.get(s"http://$localHttpbin/get", headers = Seq("x-y" -> "a", "x-y" -> "b"))
